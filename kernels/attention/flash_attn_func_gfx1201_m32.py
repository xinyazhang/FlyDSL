...
"""Flash Attention for gfx1201 with **two Q row-tiles per wave** (head_dim 64).

Variant of ``flash_attn_func_gfx1201_bp.py``. Each wave owns 32 Q rows instead
of 16 (``ROWS_PER_WAVE = WMMA_M * Q_TILES``, ``BLOCK_M = 256``), so a single
K or V operand feeds ``Q_TILES`` WMMAs instead of one. Three effects compound:

  * GEMM1 -- one K pack feeds both row-tiles' S accumulators
  * GEMM2 -- one V pack feeds both row-tiles' O accumulators
  * BLOCK_M 128 -> 256 halves the grid, so K/V global traffic halves too

Measured at head_dim=64, B=2 H=12: +14% at N=4096 (84.3 -> 96.2 TFLOPS),
+14% at N=8192, +7% at N=2048.

**head_dim 64 only.** Doubling the per-wave state costs
``o_accs`` + ``q_b_packs`` + ``s_accs`` VGPRs, which scales with head_dim:
at 64 that is +64 (183 VGPRs total, no spills, 8 waves/SIMD); at 128 it is +112,
which hits the 256-VGPR cap and spills -- measured at -27%. See
``gfx1201_fmha.md`` for the register budget curve.
...
Block:  (256,) -- 8 waves x 32 threads/wave, 32 Q rows each.
"""
...
KERNEL_NAME = "flash_attn_func_gfx1201_m32_kernel"
...
def build_flash_attn_func_m32_module_primary(
    num_heads,
    head_dim,
    causal=True,
    dtype_str="bf16",
    ...
    WMMA_K = 16
    K_SUB_N = 32
    Q_TILES = 2  # Q row-tiles owned by each wave
    ROWS_PER_WAVE = WMMA_M * Q_TILES
    ...
    # head_dim 64 only: at 128 the doubled per-wave state (o_accs 64 +
    # q_b_packs 32 + s_accs 16 = +112 VGPRs) blows past the 256-VGPR cap and
    # spills, measured at -27%. At 64 it costs exactly +64, which the register
    # budget curve prices at about -2.5%.
    if head_dim != 64:
        raise ValueError(f"m32 variant supports head_dim 64 only, got {head_dim}")
    if (BLOCK_M, BLOCK_N) != (256, 32):
        raise ValueError(f"m32 variant supports BLOCK_M=256, BLOCK_N=32 only, got {(BLOCK_M, BLOCK_N)}")
    ...
    def flash_attn_func_m32_kernel(
        Q: fx.Pointer,
        K: fx.Pointer,
        V: fx.Pointer,
        ...
        q_rows = [
            ...
            for qt in range_constexpr(Q_TILES)
        ]
        ...
        q_b_packs_all = []
        for qt in range_constexpr(Q_TILES):
            _in = arith.cmpi(arith.CmpIPredicate.slt, _raw(q_rows[qt]), _raw(seq_len_v))
            _safe = fx.Index(ArithValue(_in).select(q_rows[qt], fx.Index(0)))
            _packs = []
            for ks in range_constexpr(K_STEPS_QK):
                q_col = fx.Index(ks * K_STEP_QK) + klane * WMMA_LANE_K
                raw = load_global_v8f16(q_ptr, global_idx(_safe, q_col))
        ...
        init_args = []
        for _ in range_constexpr(Q_TILES):
            init_args.append(_raw(c_neg_inf))
            init_args.append(_raw(c_zero_f))
        for _ in range_constexpr(Q_TILES * D_CHUNKS):
        ...
        for kv_block_start, inner_iter_args in range(
            ...
            _ML = 2 * Q_TILES
            _OFF = _ML + Q_TILES * D_CHUNKS
            m_run = [inner_iter_args[2 * qt] for qt in range_constexpr(Q_TILES)]
            l_run = [inner_iter_args[2 * qt + 1] for qt in range_constexpr(Q_TILES)]
            o_accs_all = [
                [inner_iter_args[_ML + qt * D_CHUNKS + i] for i in range_constexpr(D_CHUNKS)]
                for qt in range_constexpr(Q_TILES)
            ]
            _k_vecs_cur = [inner_iter_args[_OFF + b] for b in range_constexpr(NUM_BATCHES_KV)]
            _v_vecs_cur = [
                inner_iter_args[_OFF + NUM_BATCHES_KV + b] for b in range_constexpr(V_TR_LOADS)
            ]
            ...
            # Each K pack feeds both row-tiles' S accumulators: one LDS read
            # now serves Q_TILES WMMAs instead of one. That reuse is the point.
            s_accs_all = [
            ...
            # Softmax is per Q row-tile; each tile keeps its own running
            # max/sum and its own O accumulators.
            m_new_all, l_new_all, p_vals_all = [], [], []
            for qt in range_constexpr(Q_TILES):
                s_accs = s_accs_all[qt]
                o_accs = o_accs_all[qt]
            ...
            for qt in range_constexpr(Q_TILES):
                ...
                p_packs_per_tile.append(p_packs_all)
            ...
            for pks in range_constexpr(PV_K_STEPS):
                for dc in range_constexpr(D_CHUNKS):
                    ...
                    # One V operand, Q_TILES WMMAs: halves V LDS reads per FLOP.
                    for st_idx in range_constexpr(N_SUB_TILES):
                        for qt in range_constexpr(Q_TILES):
                            o_accs_all[qt][dc] = wmma_acc(
                                cur_v_packs[st_idx],
                                p_packs_per_tile[qt][st_idx][pks],
                                o_accs_all[qt][dc],
                            )
            ...
            for qt in range_constexpr(Q_TILES):
                _yield_args.extend(o_accs_all[qt])
            for batch in range_constexpr(NUM_BATCHES_KV):
        ...
        for qt in range_constexpr(Q_TILES):
            ...
            o_finals = [
                loop_results[_ML + qt * D_CHUNKS + dc] for dc in range_constexpr(D_CHUNKS)
            ]
            ...
            q_in_bounds = q_in_bounds_all[qt]
            q_row = q_rows[qt]
            if q_in_bounds:
                for dc in range_constexpr(D_CHUNKS):
    ...
    def launch_flash_attn_m32(
        ...
        launcher = flash_attn_func_m32_kernel(Q, K, V, O, seq_len)
...
build_flash_attn_func_m32_module = build_flash_attn_func_m32_module_primary
