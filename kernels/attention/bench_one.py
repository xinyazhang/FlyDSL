...
Usage: python3 bench_one.py <baseline|bp|m32> <0|1>
"""
import sys, torch
...
BATCH, H, N = 2, 12, 4096
torch.manual_seed(0)
q, k, v = (torch.randn((BATCH, N, H, d), dtype=torch.float16, device="cuda") for _ in range(3))
...
print(f"{name:>8} causal={int(causal)}  {fl / ms * 1e-9:7.1f} TFLOPS")
