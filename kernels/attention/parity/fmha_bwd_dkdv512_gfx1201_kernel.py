...
"""dK / dV backward attention for gfx1201 at **wide head dims** (192--512).

A variant of `fmha_bwd_dkdv_gfx1201_kernel.py`, which caps at head_dim 256.
Everything below the two sections marked "WIDE" is that kernel unchanged; read
it first. This file exists because head_dim 512 does not fit *either* budget and
the two fixes interact, so they could not be added as knobs to a kernel whose
default path must stay byte-identical.

--- The two walls at head_dim 512 -----------------------------------------

Measured VGPR counts (ISA, `vgpr_count` / `vgpr_spill_count`) give
`vgpr = state + 62` where `state = packs + accumulators`, and the model
reproduces the spill counts it can be checked against (D=256 unsplit: predicts
190, ISA reports 203). Per lane, wave32, one wave owning 16 KV rows:

    an f16 operand tile   16 x D halves / 32 lanes = D/4 VGPRs
    an f32 accumulator    16 x D floats / 32 lanes = D/2 VGPRs

    head_dim 512, 2-way split    packs 128 + accs 256 = 384  (+62 = 446 / 256)
    head_dim 512, LDS            four staging tiles          = 90496 / 65536

Neither is reachable by tuning: BLOCK_M is already at the WMMA tile floor of
16, and `BLOCK_M`/`BLOCK_N` do not appear in either expression.

--- WIDE 1: `CONTRACTION_SHARDS` -- the register fix -----------------------

The base kernel's `split_head_dim` pairs the waves by *role*: role 0 holds K
and computes all of S, role 1 holds V and computes all of dP, and each owns
half the d range of both accumulators. Packs stay at `D/4` because S contracts
over the whole head dim, so the pair cannot get below `0.75 * D`.

Sharding the *contraction* as well cuts the packs. With `C` shards and the same
two roles, a team is `W = 2C` waves and wave `(c, r)` holds only
`K[:, c*D/C : (c+1)*D/C]` (r=0) or the V equivalent (r=1):

    packs  D/(4C)      accs  D/(2C)      state = 0.75 * D / C

    C = 1   384   (the base kernel's split, reproduced exactly)
    C = 2   192   (+62 = 254, fits)
    C = 4    96   (+62 = 158)

Each wave now produces a *partial* S or dP, so the base kernel's two-slot
exchange becomes a `W`-slot reduction: every wave writes its partial, reads all
`W` slots and sums the two that make S and the two that make dP. The exchange
region does not grow -- it is one slot per wave either way,
`NUM_WAVES * WARP_SIZE * 8 * 4` bytes.

--- WIDE 2: `TRANSPOSED_SOURCE` -- the LDS fix -----------------------------

The base kernel stages Q and dO in both orientations, four tiles. The
transposed pair is *not* performing a transpose: `global_load_tr_b128` already
did that on the way in from global. It is a redistribution shuffle, because the
cooperative load's block-to-wave assignment does not match the d-ownership the
GEMMs consume by.

So the transposed operand can be read straight out of the row-major tile
instead -- the same eight halves, strided by the row-major pitch rather than
contiguous. gfx1201 has no `ds_load_tr16_b128` (verified against llvm-mc;
gfx1250 does), so that costs eight `ds_read_u16` where the tile costs one
`ds_read_b128`. This is exactly `fmha_bwd_dq_gfx1201_kernel`'s
`kt_lds_layout="scalar"` path, in the other kernel.

    "tile"      four tiles; what the base kernel does
    "derived"   two tiles, transposed operands read strided

At head_dim 512 "derived" is what makes it fit:

    tile      33024 (row-major) + 40960 (transposed) + 16512 = 90496   over
    derived   33024             +     0              + 16512 = 49536   fits

The cost is issue slots, not capacity: LDS reads per WMMA go from ~1.0 to ~4.5,
and dq -- which already reads this way -- was measured LDS-bound at 3.3. Hence
the knob rather than a rewrite: at head_dim <= 256 "tile" still wins and this
kernel is not the default path.
...
- No bias. `BIAS_TYPE` has no dB output here and AOTriton computes it in a
  separate kernel.
- Head-dimension sharding *is* implemented here; see WIDE 1 above. It is the
  base kernel that lacks it.
...
def build_bwd_dkdv_module_primary(meta: BwdDkDvMetadata, knobs: BwdDkDvKnobs):
    ...
    # ---- WIDE: team geometry ----
    #
    # A *team* of `TEAM_WAVES = 2 * CONTRACTION_SHARDS` waves cooperates on one
    # set of 16 KV rows: two roles (S-producer holding K, dP-producer holding V)
    # times `C` shards of the contraction. `C == 1` is the base kernel's
    # `split_head_dim` pair exactly, so this file reproduces it rather than
    # forking a second code path.
    ...
    assert SPLIT_HEAD_DIM, "this kernel is the split path; head_dim <= 256 belongs in the base kernel"
    assert (
    ...
    TEAM_WAVES = 2 * CONTRACTION_SHARDS
    ...
    assert BLOCK_DMODEL == BLOCK_DMODEL_V, (
        f"the split gives both roles one operand array and one LDS stride, "
    ...
    # WIDE: under "derived" the two transposed tiles are not allocated at all
    # and their operands are read strided out of the row-major pair. Sizes go to
    # zero rather than the bases being skipped, so every downstream offset
    # expression stays the one the base kernel has.
    QRM_BASE = 0
    ...
    # WIDE: the team reduction. LDS is typed in 16-bit elements but `lds_f32_*`
    # index in f32; the _F32 names are the indexing unit and PDS_ELEMS the block
    # size. Mixing them is a silent 2x overrun.
    #
    # One slot per wave, whatever the team width -- `C=1` gives the base
    # kernel's two-slot exchange and `C=2` four slots over half as many teams,
    # so the region is `NUM_WAVES` slots either way.
    PDS_BASE = (ROWQ_BASE + ROWQ_ELEMS + 7) // 8 * 8
    PDS_SLOT_F32 = WARP_SIZE * 8
    PDS_TEAM_F32 = TEAM_WAVES * PDS_SLOT_F32
    PDS_ELEMS = NUM_TEAMS * PDS_TEAM_F32 * 2
    PDS_BYTE0 = PDS_BASE * 2
    _ROWQ_BATCHES = (BLOCK_M + BLOCK_SIZE - 1) // BLOCK_SIZE
    LDS_TOTAL = PDS_BASE + PDS_ELEMS

    # WIDE: a wave's share of the accumulators is 1/TEAM_WAVES of the d range,
    # and its share of the contraction is 1/CONTRACTION_SHARDS of it.
    assert (
    ...
    # head_dim == head_dim_v is asserted above, so one shard width serves both.
    ...
    def bwd_dkdv_kernel(
        ...
        # WIDE: wave -> (team, slot), then slot -> (shard, role).
        #
        #   role  0 = holds K and produces a partial S, 1 = holds V, partial dP
        #   shard which 1/C of the contraction this wave reduces over
        #   slot  which 1/TEAM_WAVES of the d range it accumulates dK/dV for
        #
        # The accumulator partition is keyed on `slot`, not on `role` or
        # `shard`, because it just has to be a partition -- every wave owns a
        # distinct d window and the three indices need not agree about which.
        group_of_wave = wave_id // fx.Index(TEAM_WAVES)
        _slot = wave_id % fx.Index(TEAM_WAVES)
        role = fx.Int32(_slot % fx.Index(2))
        _shard_i32 = fx.Int32(_slot // fx.Index(2))
        ...
        _shard_k0 = _shard_i32 * fx.Int32(D_STEPS_SHARD)
        ...
        # Role 0 holds K (for S), role 1 holds V (for dP), and WIDE 1 cuts that
        # again to the wave's own contraction shard -- `D/(4C)` registers. The
        # role is resolved ONCE, here, on plain values: an aperture method call
        # inside a ternary arm is `name.method(...)` under a dynamic branch, the
        # carried-state trap `fmha_common`'s header documents, and it surfaces
        # as a bad_alloc out of `get_ir_types` rather than the documented
        # UnboundLocalError. An !llvm.ptr cannot cross an scf.if result either,
        # so the pointer is chosen as an i64 address and rebuilt.
        ...
        # Only this shard's slice of the contraction. `_shard_k0` is dynamic, so
        # the column is built per step rather than folded into the constexpr.
        my_packs = []
        for ks in range_constexpr(D_STEPS_SHARD):
            _c = fx.Index((_shard_k0 + fx.Int32(ks)) * fx.Int32(WMMA_K)) + klane * WMMA_LANE_K
            my_packs.append(k_ap.read_v8(fetch_my, _kv_safe, _c, _kv_in))
        ...
        for _it, _iter_args in range(fx.Index(0), fx.Index(_total_i32), 1, init=init_args):
            ...
            if const_expr(KEEP_TR_TILES):
                # WIDE 2: under "derived" there is no transposed tile to fill,
                # and the `global_load_tr_b128` fetches that fed it go with it --
                # the same eight halves are read out of the row-major tile at
                # GEMM time instead.
            ...
            for qsub in range_constexpr(Q_SUBTILES):
                ...
                # WIDE 1. One GEMM per wave, over this wave's shard of the
                # contraction only: role 0 makes a partial S, role 1 a partial
                # dP. Same shape and accumulator, different operands, and both
                # were selected in the prologue -- so this loop is branch-free
                # and `_rm_base_off` is what points it at Q or dO.
                ...
                for ks in range_constexpr(D_STEPS_SHARD):
                    ...
                    _mine = fmha.wmma_acc(_a_op, my_packs[ks], _mine)

                # The team reduction. Every wave writes its partial and then
                # reads all `TEAM_WAVES` slots: `slot = 2*shard + role`, so the
                # even slots sum to S and the odd ones to dP. Reading back one's
                # own slot rather than keeping it in registers costs one LDS
                # read and makes every wave's arithmetic below identical, which
                # is what keeps the P/dS block free of role branches.
                ...
                for _si in range_constexpr(8):
                    ...
                    for _c_sh in range_constexpr(CONTRACTION_SHARDS):
                        ...
                        _dv_c = fmha.lds_f32_load(
                            _lds_byte_base, PDS_BYTE0, _pds_team + fx.Int32((2 * _c_sh + 1) * PDS_SLOT_F32 + _si)
                        )
                ...
                for si in range_constexpr(8):
                    ...
                    # WIDE 1: the team reduction already gave every wave the
                    # full S and dP, so there is no role selection left here.
                ...
                # WIDE 2: one transposed A operand, from whichever source the
                # knob selects. "tile" reads the eight halves contiguously out
                # of the transposed tile; "derived" reads the same eight out of
                # the row-major tile, strided by its pitch, which is why the
                # transposed tile need not exist. `rm_ap`/`tr_ap` and the base
                # differ per tensor, so both are parameters.
