...
"""Paired-wave dK/dV for gfx1201 -- a prototype, not a shipping path yet.

The register-split variant of `fmha_bwd_dkdv_gfx1201_kernel.py`, derived from
it and reusing its addressing, apertures and epilogue unchanged. Read that file
first; this header covers only the difference.

The baseline gives every wave 16 KV rows and the *whole* head dim, so a wave
carries `k_packs + v_packs + dk_accs + dv_accs` = **1.5 x head_dim** VGPRs. That
crosses the 256-register file at head_dim 192 and spills 251 registers at 256,
where throughput is 14% of the WMMA ceiling against the forward's 43%.

Two waves here share a 16-row KV group and split it by role:

    role 0   S  = Q  . K^T    holds K
    role 1   dP = dO . V^T    holds V
    both     accumulate one HALF of the head dim of dK and dV

    per lane: D/4 (own operand) + D/4 (dk half) + D/4 (dv half) = 0.75 x D,
    the forward's ratio.

The halves are **disjoint output columns**, so there is no cross-wave
reduction. This is not the Q-stream split the baseline's header rejects for
needing a `BLOCK_N x head_dim` f32 reduction through LDS.

Each wave writes its own raw accumulator to LDS and reads its partner's; after
one barrier both hold S and dP and derive P and dS identically, so no branch is
needed in the arithmetic. The cost is `BLOCK_N = 16 * (num_waves/2)`, half the
baseline's, so Q/dO stream past twice as often.

Measured, B=4 H=16 N=2048 f16, ceiling 212.8 TFLOPS:

    head_dim  waves   base       paired    gain   base%  paired%
    128       8        3.05 ms    3.62 ms  0.84x   42%    36%
    128       16       3.61       3.78     0.96x   36%    34%
    192       8        7.75       8.39     0.92x   25%    23%
    192       16       7.61       6.99     1.09x   25%    28%
    256       8       18.80       8.17     2.30x   14%    32%
    256       16      19.04       7.81     2.44x   14%    33%

    registers  base                  paired
    192        vgpr 256, spill 91    vgpr 206, spill 0
    256        vgpr 256, spill 251   vgpr 254, spill 0

**It wins only where the baseline spills.** At 128 the baseline has none, so the
exchange and the halved BLOCK_N cost 4-16% for nothing. At 256 the spills are
the whole bottleneck and removing them is worth 2.4x -- 14% to 33% of peak,
which is what head_dim 128 already reaches. A head_dim-gated alternative, not a
replacement.

Two traps, both hit while writing this:

**No aperture method call inside a ternary arm.** The rewriter makes each arm a
branch region, so `k_ap.read_v8(...) if role0 else v_ap.read_v8(...)` is
`name.method(...)` under a dynamic branch -- the carried-state trap
`fmha_common`'s header documents. It surfaces as `MemoryError: std::bad_alloc`
out of `get_ir_types`, which looks nothing like the documented symptom. The fix
is to resolve the role once in the prologue, on plain values. An `!llvm.ptr`
cannot cross an `scf.if` result either, so the pointer is selected as an i64
address and rebuilt.

**Two unit systems meet in the exchange.** The LDS block is typed in 16-bit
elements but `lds_f32_store/load` index in f32. Sizing in one and indexing in
the other was a silent 2x overrun that produced plausible garbage -- rel 0.2 to
0.66, not a crash.

Not done: nothing dispatches to this; `num_waves` is unswept against the halved
BLOCK_N; `head_dim == head_dim_v` is assumed by the shared operand loop and LDS
stride but not asserted; `v_ap` and the `fetch_k`/`fetch_v` closures are now
dead but left in place -- removing them broke the file once and the cleanup is
not worth a second attempt before the design is settled.
"""
...
def build_bwd_dkdv_module_primary(meta: BwdDkDvMetadata, knobs: BwdDkDvKnobs):
    ...
    assert NUM_WAVES % 2 == 0, f"paired dK/dV needs an even wave count, got {NUM_WAVES}"
    NUM_PAIRS = NUM_WAVES // 2
    ...
    # LDS is typed in 16-bit elements but `lds_f32_*` index in f32; the _F32
    # names are the indexing unit and PDS_ELEMS is the block size.
    PDS_BASE = (ROWQ_BASE + ROWQ_ELEMS + 7) // 8 * 8
    PDS_SLOT_F32 = WARP_SIZE * 8
    PDS_PAIR_F32 = 2 * PDS_SLOT_F32
    PDS_ELEMS = NUM_PAIRS * PDS_PAIR_F32 * 2
    PDS_BYTE0 = PDS_BASE * 2
    _ROWQ_BATCHES = (BLOCK_M + BLOCK_SIZE - 1) // BLOCK_SIZE
    LDS_TOTAL = PDS_BASE + PDS_ELEMS
    assert D_STEPS % 2 == 0 and DV_STEPS % 2 == 0, "paired dK/dV needs even D_STEPS/DV_STEPS"
    D_STEPS_HALF = D_STEPS // 2
    DV_STEPS_HALF = DV_STEPS // 2
    ...
    def bwd_dkdv_kernel(
        ...
        pair_id = wave_id // fx.Index(2)
        role = fx.Int32(wave_id % fx.Index(2))
        _half_i32 = role * fx.Int32(D_STEPS_HALF)
        _vhalf_i32 = role * fx.Int32(DV_STEPS_HALF)
        kv_row_in_tile = pair_id * fx.Index(ROWS_PER_WAVE) + lane16
        kv_row_abs = start_k + kv_row_in_tile
        kv_row_abs_i32 = start_k_i32 + fx.Int32(pair_id) * fx.Int32(ROWS_PER_WAVE) + fx.Int32(lane16)
        _pds_pair = fx.Int32(pair_id) * fx.Int32(PDS_PAIR_F32) + fx.Int32(lane) * fx.Int32(8)
        ...
        # Role resolved ONCE, on plain values. An aperture method call inside a
        # ternary arm is the carried-state trap `fmha_common` documents; it fails
        # with bad_alloc out of get_ir_types. An !llvm.ptr cannot cross an scf.if
        # result either, so the pointer is chosen as an i64 and rebuilt.
        ...
        init_args = []
        for _ in range_constexpr(D_STEPS_HALF):
            init_args.append(fx.as_ir_value(c_zero_v8f32))
        for _ in range_constexpr(DV_STEPS_HALF):
            init_args.append(fx.as_ir_value(c_zero_v8f32))
        ...
        for _it, _iter_args in range(fx.Index(0), fx.Index(_total_i32), 1, init=init_args):
            dk_accs = [_iter_args[i] for i in range_constexpr(D_STEPS_HALF)]
            dv_accs = [_iter_args[D_STEPS_HALF + i] for i in range_constexpr(DV_STEPS_HALF)]
            ...
            for qsub in range_constexpr(Q_SUBTILES):
                ...
                # One GEMM per wave: role 0 makes S, role 1 makes dP. Operands
                ...
                # Relay: write mine, read the partner's. After the barrier both
                # waves hold S and dP, so the P/dS math below needs no branch.
                ...
                for si in range_constexpr(8):
                    ...
                    _s = fastmath.mul(_mv if _role0 else _ov, sm_log2e)
                ...
                for dc in range_constexpr(DV_STEPS_HALF):
                ...
                for dc in range_constexpr(D_STEPS_HALF):
        ...
        if _row_live:
            ...
            # Each wave stores only the head-dim half it accumulated; the pair
            # writes disjoint columns, hence no reduction.
            for dc in range_constexpr(D_STEPS_HALF):
            ...
            for dc in range_constexpr(DV_STEPS_HALF):
                _t = Vec(loop_results[D_STEPS_HALF + dc]).to(elem_dtype).ir_value()
