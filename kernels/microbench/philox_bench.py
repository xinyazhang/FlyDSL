...
def build(width, n_rounds=DEFAULT_ROUNDS, calls=CALLS_PER_THREAD):
    ...
    def k(OUT: fx.Pointer, seed: fx.Int64, base: fx.Int64):
        ...
        for c in range_constexpr(calls):
            off = base + blk * fx.Int64(THREADS * calls) + tid * fx.Int64(calls) + fx.Int64(c)
            vals = rng.u32(seed, off)
            for j in range_constexpr(rn):
                acc = acc ^ fx.Int32(vals[j])
        out = fx.recast_iter(i32p, OUT)
        fx.ptr_store(acc, out + fx.Int64(fx.Int32(blk) * fx.Int32(THREADS) + fx.Int32(fx.Index(gpu.thread_idx.x))))

    @flyc.jit
    def launch(OUT: fx.Pointer, seed: fx.Int64, base: fx.Int64, stream: fx.Stream = fx.Stream(None)):
        k(OUT, seed, base).launch(grid=(fx.Index(BLOCKS), 1, 1), block=(THREADS, 1, 1), stream=stream)
...
def vgpr_count(width, calls=1):
    ...
    with tempfile.TemporaryDirectory() as d:
        ...
        subprocess.run(
            [sys.executable, __file__, "--dump-only", str(width), str(calls)], env=env, capture_output=True, timeout=600
        )
...
def main():
    ...
    print(f"Philox, {BLOCKS} blocks x {THREADS} threads x {CALLS_PER_THREAD} calls, " f"{DEFAULT_ROUNDS} rounds\n")
    ...
    for w in PHILOX_WIDTHS:
        ...
        res[w] = (grs, v)
        print(
            f"{w:>6} {Philox(width=w).randoms_per_offset:>9} {ms:>8.3f} {grs:>10.2f} "
            f"{(v if v is not None else '?'):>11}"
        )
    a, b = res[32][0], res[64][0]
    print(f"\n32-bit is {a / b:.2f}x the throughput of 64-bit per random")
    print(f"VGPR delta (64 - 32): " f"{res[64][1] - res[32][1] if None not in (res[32][1], res[64][1]) else '?'}")
    return 0
